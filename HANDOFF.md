...
> **2026-09-02現在の注意**：このファイル後半には初期開発時の履歴も残っている。最新の実装状況と作業ログは `docs/SESSION_NOTES.md`、公開判定は `src/data/quality-gates.mjs`、科目構成は `src/data/curriculum.ts` を正とする。現行の作業ツリーは `C:\Users\sotar\Documents\Codex\2026-07-24\a\work\topica`。公開サイトは7科目の体系記事、学習ガイド、用語20件、詳細判例85件を収録し、直近ビルドは560ページ、Pagefindは441ページを索引化している。2026-09-02までに百選級の重点監査対象判例15件を追加し、最新デプロイ `fbfdb486` まで本番確認済み。
...
### 行政法（`gyosei`）— **基礎編19本＋応用編14本 完全完成・公開済み**
第1編から第6編まで、基礎編19本と応用編14本の計33本が完成し、本番公開済み（2026-07-24）。行政作用、行政手続、行政争訟、国家賠償、損失補償について、基礎知識から事例適用まで体系的に収録。
