...
### 行政法（`gyosei`）— **基礎編12/19本 実装済み**
第1・2編に加え、第3編3本・第4編2本が完成し、第5編1本目 `fufuku-shinsa`（行政不服審査法の仕組み）まで実装済み（2026-07-24）。次は `shobunsei`（取消訴訟の訴訟要件①——処分性）。公開状況は `docs/SESSION_NOTES.md` の最新ログを参照。
