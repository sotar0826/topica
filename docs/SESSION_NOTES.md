...
**次セッションで最初にやること／持ち越し**:
1. Tier 2 残り：`keiyaku-soron-ouyou`（定型約款・契約締結上の過失。判例候補：最判昭59.9.18／平23.4.22 → 要照合）
2. Tier 3：`bensai-ouyou`（478条）／`shutoku-jiko-ouyou`／`sagi-kyohaku-ouyou`
3. コラム：時効と登記／416条の射程
4. 民法完了後：商法の科目設計（curriculum.ts に `shoho` 追加・単元構成案をユーザーに提案）
5. Search Console：インデックス登録リクエスト（クォータ回復後）・sitemapステータス確認
