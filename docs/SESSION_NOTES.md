...
**次セッションで最初にやること／持ち越し**:
1. CONTENT_PLAN 残り1本：`sagi-kyohaku-ouyou`（★☆☆。96条3項が既存記事と重複大→書く場合は第三者詐欺・強迫の絶対的保護に絞る。判例候補：最判昭49.9.26〔96条3項の善意〕等 → 要照合）
2. **商法の科目設計**：curriculum.ts に `shoho` 追加。構成案をユーザーに提案してから着手（ユーザーは「編・章分け」を想定。より良い案があれば提案せよとのこと）
3. Search Console：インデックス登録リクエスト（クォータ回復後）・sitemapステータス確認・新記事12本の登録
4. 判例照合の再現手順は本ファイル 2026-07-05 エントリの技術メモ参照
