...
- **行政法は基礎編12/19本を公開済み**。第3編・第4編まで完成し、第5編は `fufuku-shinsa` まで公開。次は `shobunsei`（取消訴訟の訴訟要件①——処分性）
...
- **AdSense審査中**（ca-pub-7356455610185406。スニペット＋ads.txt設置済み。結果待ち→承認されたら広告配置の設定、非承認なら理由に対処して再申請）
...
4. **運用**: AdSense審査結果への対応／Search Consoleの週次確認（カバレッジ増・CTR低いページのseoTitle改善）／判例解説の追加収録（台帳には300件以上照合済みがあり、50件しかページ化していない）
