...
4. **運用**: AdSense再審査結果への対応／Search Consoleの週次確認（全50判例のsitemap反映、カバレッジ増、CTR低いページのseoTitle改善）／簡易用語20件の拡充

## §10 判例解説「百選級」増強の仕様（ユーザー承認済み・全50件完了）
...
- **全50件への展開完了**: 2026-08-04に残り18件を改稿。17件は`fetch-zenbun.py`で全文ページを追加し、量的過剰防衛事件のみ特殊フォントのため裁判要旨ベースとした。全文ページは累計47件
